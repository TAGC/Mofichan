...
namespace Mofichan.Tests
{
    public class BaseReflectionBehaviourTests
    {
        ...
        private class MalformedReflectionBehaviour : BaseReflectionBehaviour
        {
            public MalformedReflectionBehaviour() : base(() => Mock.Of<IResponseBuilder>())
            {
            }

            [AuthorisationIncomingMessageFilter(UserType.Adminstrator, "Shouldn't be called anyway")]
            public void Malformed_Request_Handler(IncomingMessage message)
            {
                throw new ArgumentException("Should not have been invoked - void return type");
            }

            [AuthorisationIncomingMessageFilter(UserType.Adminstrator, "Shouldn't be called anyway")]
            public OutgoingMessage? Malformed_Request_Handler_2()
            {
                throw new ArgumentException("Should not have been invoked - missing message parameter");
            }

            [AuthorisationIncomingMessageFilter(UserType.Adminstrator, "Shouldn't be called anyway")]
            public OutgoingMessage Malformed_Request_Handler_3(IncomingMessage message)
            {
                throw new ArgumentException("Should not have been invoked - return type is not nullable");
            }
        }
        #endregion
        ...
        [Fact]
        public void Reflection_Behaviour_Should_Ignore_Methods_With_Invalid_Signatures()
        {
            // GIVEN an instance of a malformed reflection behaviour.
            var behaviour = new MalformedReflectionBehaviour();

            // GIVEN a message from an administrative user.
            var mockFrom = new Mock<IUser>();
            mockFrom.SetupGet(it => it.Type).Returns(UserType.Adminstrator);
            mockFrom.SetupGet(it => it.Name).Returns("Stewart Mockington");
            ...
            // EXPECT no methods declared in the malformed behaviour are invoked.
            /*
             * An exception will be thrown by the methods if they are.
             */
            behaviour.OnNext(message);

            // EXPECT no responses were generated by the behaviour.
